...
sub SaveGlobals {
    ...
    # BNC #484736: Some forwarders are defined, forwarders file has to be included
    if ($forwarders_defined && !$forwarders_included) {
	y2milestone("Including forwarders file ".$forwarders_include);
	push @options, { "key" => "include", "value" => $forwarders_include_record };
    # No forwarders defined, remove them from include
    } elsif (!$forwarders_defined) {
	my $remove_includes = 1;

	my @tmp_options = ();
	# Removing the file inclusion
	foreach my $option (@options) {
	    if ($option->{"key"} eq "include" && $option->{"value"} eq $forwarders_include_record) {
		y2milestone ("Removing include ".$forwarders_include_record);
	    } else {
		# There are other includes, do not remove them completely
		$remove_includes = 0 if ($option->{"key"} eq "include");
		push @tmp_options, $option;
	    }
	}
	@options = @tmp_options;

	# Removes the include only if it was alone, otherwise the other ones are left
	if ($remove_includes) {
	    y2milestone ("All includes will be removed from options {...}");
	    push @del_options, "include";
	}
    }
    ### <-- Handling 'forwarders'
